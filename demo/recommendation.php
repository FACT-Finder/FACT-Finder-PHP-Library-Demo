...
// We only need on Recommendation adapter here.
$recommendationAdapter = FF::getInstance(
...
try {
	echo $recommendationAdapter->getRawRecommendations();
} catch (\Exception $e) {
	if (!headers_sent()) {
		...
		$logfile = "recommendation.error.log";
		$f = fopen($logfile, 'a');
