...
// it, we'll also configure *log4php*. Because we only want to run this function
// once (and then just want the resulting string over and over again), we pass
// the anonymous function to `$dic->share()`.
//
...
$dic['loggerClass'] = $dic->share(function($c) {
...
// own instance, so we don't use `$dic->share()`. Unless you want to do
...
// fancy, you'd be better off writing a new factory).

$dic['request'] = function($c) {
...
$dic['clientUrlBuilder'] = $dic->share(function($c) {
    return FF::getInstance(
